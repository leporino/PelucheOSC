...
//float per1, per2, per3, per4, per5, sl3, sl4, sliderValue;// midi valores
...
void ParticleSystem::draw(float per1,float per2,float per3,float per4,float per5,float sl3,float sl4,float sliderValue) {
	int n = particles.size();
	//glBegin(GL_POINTS);
	for(int i = 0; i < n; i++)
		particles[i].draw(per1,per2,per3,per4,per5,sl3,sl4,sliderValue);
	//glEnd();
}


void ParticleSystem::draw2(float sliderValue, float per3, float per4,float per5) {
	int n = particles.size();
	//glBegin(GL_LINE_STRIP);
	for(int i = 0; i < n; i++)
		particles[i].draw2(sliderValue, per3, per4, per5);
	//glEnd();
}
